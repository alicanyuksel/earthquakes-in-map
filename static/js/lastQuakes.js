function writeLastQuakes(detailsOfAllQuakes, rangeValue) {    
    ...
    let htmlContent = `<center><h5>Son ${rangeValue} Depremler</h5></center><hr>`
    for (var key in detailsOfAllQuakes.slice(null, rangeValue)) {
        ...
        if (detailsOfAllQuakes[key].magnitude > 0 && detailsOfAllQuakes[key].magnitude <= 2) {
            htmlContent += `
            <a id="linkToFly" href="#" onclick="geoToFly(${detailsOfAllQuakes[key].longitude}, ${detailsOfAllQuakes[key].latitude}); toggleNav();"><p>Lokasyon: ${detailsOfAllQuakes[key].location}</p></a>
